...
Ask it things like "what files are in this project?" or "read
Cargo.toml and tell me the package name" — it only reads and lists
files under wherever you launched it from. Ollama is chat-only for now;
file-aware tools are Mistral-only until local tool-calling is built
(see Roadmap).
